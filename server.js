...
async function startApp(){
    ...
    server.listen(port, () => {
        ...
        cron.schedule('* */12 13-20 * *', () => {
            sender.getReciveDate()
            .then(countersData =>{
                const reciveStart = countersData[0].nn_ind_receive_start;
                const reciveEnd = countersData[0].nn_ind_receive_end;
            ...
            switch(req.url){
                case '/send':{
                    ...
                    req.on('data', data =>{
                        ...
                        fs.appendFileSync(logs, `${moment().format('lll')} :: Successfully recived data ${stats} from spreadSheet \r\n`, {format: 'a+'});
